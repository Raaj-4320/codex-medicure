...
const seedMedicines: SeedMedicine[] = [
  { id: 'mm-paracetamol', brandName: 'Paracetamol', genericName: 'Paracetamol', category: 'Analgesic', manufacturer: 'MediCare Labs', rxRequired: false, schedule: 'None' },
  { id: 'mm-dolo-650', brandName: 'Dolo 650', genericName: 'Paracetamol', category: 'Analgesic', manufacturer: 'Micro Labs', rxRequired: false, schedule: 'None' },
  { id: 'mm-azithromycin', brandName: 'Azithromycin', genericName: 'Azithromycin', category: 'Antibiotic', manufacturer: 'BioPharm', rxRequired: true, schedule: 'Schedule H' },
  { id: 'mm-atorvastatin', brandName: 'Atorvastatin', genericName: 'Atorvastatin', category: 'Cardiac', manufacturer: 'CardioLife', rxRequired: true, schedule: 'Schedule H' },
  { id: 'mm-metformin', brandName: 'Metformin', genericName: 'Metformin', category: 'Diabetes', manufacturer: 'Gluco Health', rxRequired: true, schedule: 'Schedule H' },
];
...
const makeSeedTraceId = (prefix: 'ord' | 'inv' | 'noti'): string =>
...
export const ensureSeedData = async (): Promise<void> => {
  try {
    ...
    if (!hasMedicineMaster) {
      console.group('[SEED][FORCED_INSERT]');
      console.log('DETAIL:', 'medicine_master is empty. Inserting required medicines.');
      console.groupEnd();
    ...
    const medicineMasterSnap = await getDocs(query(collection(db, 'medicine_master'), limit(1)));
    if (medicineMasterSnap.empty) {
      console.group('[SEED][FORCED_INSERT]');
      console.log('DETAIL:', 'medicine_master remained empty after check. Reinserting.');
      console.groupEnd();
      for (const medicine of seedMedicines) {
        await setDoc(doc(db, 'medicine_master', medicine.id), {
          ...medicine,
    ...
    if (!hasInventory) {
      const items = [
        { pharmacyId: 'seed-pharmacy-1', medicineMasterId: 'mm-paracetamol', price: 45, stock: 120 },
        { pharmacyId: 'seed-pharmacy-1', medicineMasterId: 'mm-dolo-650', price: 30, stock: 80 },
      ];
      for (const item of items) {
        await addDoc(collection(db, 'inventory'), {
          ...item,
          traceId: makeSeedTraceId('inv'),
          isVisible: true,
          isFeatured: false,
          createdAt: new Date().toISOString(),
        });
    ...
    if (!hasNotifications) {
      const notifications = [
        { userId: 'seed-seller-1', orderId: 'seed-system-1', title: 'Welcome', message: 'Your pharmacy is active', type: 'system' },
        { userId: 'seed-seller-1', orderId: 'seed-system-2', title: 'Inventory Tip', message: 'Add featured medicines', type: 'stock' },
      ];
      for (const notification of notifications) {
        await addDoc(collection(db, 'notifications'), {
          ...notification,
          traceId: makeSeedTraceId('noti'),
          isRead: false,
          createdAt: new Date().toISOString(),
        });
      }
    ...
    const seededCount = Object.values(seededFlags).filter(Boolean).length;

    logFlow('SEED_DATA', {
      expected: ['seeded required collections when empty', 'cleaned invalid records'],
      received: seededFlags,
      status: seededCount === 0 ? 'partial' : 'success',
    });
  } catch (error) {
    logFlow('SEED_DATA', {
