...
export const ensureSeedData = async (): Promise<void> => {
  try {
    ...
    if (!hasMedicineMaster) {
      ...
      logEvent('SEED.MEDICINE_MASTER.DONE', { count: seedMedicines.length });
    }
    ...
    if (!hasInventory) {
      ...
      logEvent('SEED.INVENTORY.DONE', { count: items.length });
    }
    ...
    if (!hasOrders) {
      ...
      logEvent('SEED.ORDERS.DONE', { count: orders.length });
    }
    ...
    if (!hasNotifications) {
      ...
      logEvent('SEED.NOTIFICATIONS.DONE', { count: notifications.length });
    }
    ...
    for (const inventoryDoc of inventorySnap.docs) {
      ...
      if (!data.medicineMasterId) {
        ...
        logEvent('SEED.CLEANUP.INVENTORY_REMOVED', { id: inventoryDoc.id, reason: 'missing medicineMasterId' });
      }
    }
    ...
    for (const pharmacyDoc of pharmacySnap.docs) {
      ...
      if (!data.ownerId || !data.sellerId) {
        ...
        logEvent('SEED.CLEANUP.PHARMACY_REMOVED', { id: pharmacyDoc.id, reason: 'missing owner/seller linkage' });
      }
    }

    logEvent('SEED.CHECK.COMPLETE', {});
  } catch (error) {
    logEvent('SEED.CHECK.ERROR', { error });
  }
};
