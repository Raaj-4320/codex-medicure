...
function normalizePharmacy(id: string, data: DocumentData): Pharmacy {
  ...
  const addressSource = toDictionary(source.address);
  const structuredAddress = {
    street: asString(addressSource.street),
    area: asString(addressSource.area),
    city: asString(addressSource.city),
    state: asString(addressSource.state),
    pincode: asString(addressSource.pincode),
  };
  return {
    id,
    sellerId: ownerId,
    ownerId,
    name: asString(source.name || source.pharmacyName, 'Profile Incomplete'),
    pharmacyName: asString(source.pharmacyName || source.name, 'Profile Incomplete'),
    ownerName: asString(source.ownerName || toDictionary(source.verificationDetails).ownerName),
    licenseNumber: asString(source.licenseNumber || toDictionary(source.verificationDetails).licenseNumber),
    description: asString(source.description),
    address: structuredAddress as Pharmacy['address'],
    contactNumber: asString(source.contactNumber || source.phone),
    phone: asString(source.phone || source.contactNumber),
    email: asString(source.email),
    gstNumber: asString(source.gstNumber),
...
export function isPharmacyProfileComplete(pharmacy: Partial<Pharmacy> & { verificationDetails?: Dictionary }): boolean {
  ...
  return Boolean(
    asString((pharmacy as Dictionary).pharmacyName || pharmacy.name).trim() &&
      asString((pharmacy as Dictionary).ownerName || verificationDetails.ownerName).trim() &&
      asString((pharmacy as Dictionary).licenseNumber || verificationDetails.licenseNumber || (pharmacy as Dictionary).license).trim() &&
      asString((pharmacy as Dictionary).phone || pharmacy.contactNumber).trim() &&
      asString(pharmacy.email).trim() &&
      asString(address.street).trim() &&
      asString(address.area).trim() &&
      asString(address.city).trim() &&
      asString(address.state).trim() &&
      asString(address.pincode).trim() &&
...
async function updatePharmacy(id: string, patch: Dictionary): Promise<boolean> {
  ...
  await setDoc(doc(db, 'pharmacies', id), normalizedPatch, { merge: true });
  return true;
}
...
async function createPharmacy(payload: Dictionary): Promise<Pharmacy> {
  ...
  const normalizedPayload: Dictionary = {
    ...
    ownerId,
    sellerId: ownerId,
    name: asString(payload.name || payload.pharmacyName, 'Profile Incomplete'),
    pharmacyName: asString(payload.pharmacyName || payload.name, 'Profile Incomplete'),
    ownerName: asString(payload.ownerName || toDictionary(payload.verificationDetails).ownerName),
    licenseNumber: asString(payload.licenseNumber || toDictionary(payload.verificationDetails).licenseNumber),
    description: asString(payload.description),
    address: {
      street: asString(toDictionary(payload.address).street),
      area: asString(toDictionary(payload.address).area),
      city: asString(toDictionary(payload.address).city),
      state: asString(toDictionary(payload.address).state),
      pincode: asString(toDictionary(payload.address).pincode),
    },
    contactNumber: asString(payload.contactNumber || payload.phone),
    phone: asString(payload.phone || payload.contactNumber),
    email: asString(payload.email),
    gstNumber: asString(payload.gstNumber),
        ...
        try {
          ...
          if (result && typeof (result as Promise<unknown>).then === 'function') {
            return (result as Promise<unknown>).catch((error) => {
              ...
              return getFallbackByName(String(prop));
            });
          }
          return result;
        } catch (error) {
