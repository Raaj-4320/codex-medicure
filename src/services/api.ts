...
async function createMedicine(payload: Dictionary): Promise<Dictionary> {
  ...
  if (medicinePayload.pharmacyId) {
    if (!asString(medicinePayload.medicineMasterId)) {
      logEvent('DATA.ERROR', {
        type: 'INVALID_INVENTORY_WRITE',
        reason: 'missing medicineMasterId',
        payload: medicinePayload,
      });
      throw new Error('medicineMasterId is required for inventory item');
    }
    if (typeof medicinePayload.price !== 'number' || typeof medicinePayload.stock !== 'number') {
      logEvent('DATA.ERROR', {
        type: 'INVALID_INVENTORY_WRITE',
        reason: 'missing price/stock',
        payload: medicinePayload,
      });
      throw new Error('price and stock are required for inventory item');
    }
...
async function createInventoryEntry(payload: Dictionary): Promise<SellerMedicine> {
  if (!asString(payload.pharmacyId) || !asString(payload.medicineMasterId)) {
    logEvent('DATA.ERROR', { type: 'INVALID_INVENTORY_WRITE', reason: 'missing pharmacyId or medicineMasterId', payload });
    throw new Error('pharmacyId and medicineMasterId are required');
  }
  if (typeof payload.price !== 'number' || typeof payload.stock !== 'number') {
    logEvent('DATA.ERROR', { type: 'INVALID_INVENTORY_WRITE', reason: 'missing price or stock', payload });
