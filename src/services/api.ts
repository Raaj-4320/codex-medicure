...
function normalizePharmacy(id: string, data: DocumentData): Pharmacy {
  ...
  return {
    ...
    verificationStatus: (asString(source.verificationStatus, 'pending') as Pharmacy['verificationStatus']) ?? 'pending',
...
async function getPharmacies(filters: FilterOptions = {}): Promise<Pharmacy[]> {
  return listCollection('pharmacies', normalizePharmacy, filters);
}

async function updatePharmacy(id: string, patch: Dictionary): Promise<boolean> {
  return patchDoc('pharmacies', id, { ...patch, updatedAt: nowIso() });
}
...
async function getPayouts(filters: FilterOptions = {}): Promise<SellerPayout[]> {
  const payouts = await listCollection('payouts', normalizePayout, filters);
...
async function getCompliance(filters: FilterOptions = {}): Promise<ComplianceDocument[] | { status: 'ok' }> {
  const compliance = await listCollection('compliance', normalizeCompliance, filters);
  if (compliance.length === 0 && Object.keys(filters).length === 0) {
    return { status: 'ok' };
  }
  return compliance;
}
...
async function getAnalytics(filters: FilterOptions = {}): Promise<AnalyticsData> {
  const orders = await getOrders(filters.sellerId ? { pharmacyId: asString(filters.sellerId) } : {});
  if (orders.length === 0) {
